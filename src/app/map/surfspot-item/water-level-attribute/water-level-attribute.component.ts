...
export class WaterLevelAttributeComponent implements OnInit {
  ...
  getIndicatorCssClasses(): string[] {
    ...
    if (
      this.hydroDetails.value != -1 &&
      new Date(this.hydroDetails.measuredAt) >= lastValidMeasurementTime
    ) {
      if (
        this.surfSpot.bmSpotInfo.minimalWaterLevel <= this.hydroDetails.value
      ) {
