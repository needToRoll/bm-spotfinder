...
export class HydroDataProviderService {
  ...
  constructor(private _wls: FirebaseWaterLevelService) {
    ...
    this._wls.getAllCurrentWaterLevels().subscribe(wls => this.walterLevels.next(wls))
    this._wls.getAllHydroDataSources().subscribe(hds => this.hydroSources.next(hds))
  }

  public readHydroDataForSpot(spot: SurfSpot): Observable<HydroDetails> {
    let result: HydroDetails = {value: -1, measuredAt: "", sourceLink: "", sourceName: ""};
    return this.walterLevels.pipe(
      withLatestFrom(this.hydroSources)
    ).pipe(map(([currentWaterLevels, currentSources]) => {
      if (spot.bmSpotInfo.waterLevelSource && spot.bmSpotInfo.waterLevelSource.length != 0) {
        let cwl = currentWaterLevels.find(wl => wl.id == spot.bmSpotInfo.waterLevelSource);
        if (cwl) {
