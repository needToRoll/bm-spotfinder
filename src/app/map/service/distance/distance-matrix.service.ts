...
export class DistanceMatrixService {
   ...
   * @param surfSpots
   */
  public calculateDistanceFromOriginToSpots(
    ...
    if (localStorage.getItem(cacheKey) != null) {
      try {
        ...
        spotsFromCache = cachedValues.filter((value) =>
          surfSpots.map((s) => s.placeId).includes(value.placeId)
        );
        spotsToCalculate = surfSpots.filter(
          (value) => !cachedValues.map((s) => s.placeId).includes(value.placeId)
        );
      } catch (e) {
        localStorage.removeItem(cacheKey);
      }
    }
    return this._calculateDistanceFromOriginToSpots(origin, spotsToCalculate)
      .pipe(
        map((spotsWithDistanceCalculatedByRemote) =>
          spotsWithDistanceCalculatedByRemote.concat(spotsFromCache)
        ),
        map((allSpots) =>
          allSpots.sort(
            (one, other) => this.spotDistance(one) - this.spotDistance(other)
          )
        )
      )
      .pipe(
        tap((allSortedSpots) =>
          localStorage.setItem(
            cacheKey,
            this._buildJsonCacheString(allSortedSpots, cachedValues)
          )
        )
      );
  }
  ...
  private _cleanCacheIfNeeded(...originsToReserve: GoogleCoordinates[]) {
    ...
    let keysToPreserve = originsToReserve.map((value) =>
      this.toLocalStorageKey(value)
    );
    for (let localStorageKey in localStorage) {
      if (
        localStorageKey.startsWith(this.BM_SPOT_CACHE_DISTANCE) &&
        !keysToPreserve.includes(localStorageKey)
      ) {
        potentiallyObsoleteKeys.push(localStorageKey);
      }
    }
    if (
      potentiallyObsoleteKeys.length != 0 &&
      potentiallyObsoleteKeys.length >
        maxNumberOfLocationToCache - keysToPreserve.length
    ) {
      potentiallyObsoleteKeys.forEach((key) => localStorage.removeItem(key));
    }
  }

  private _buildJsonCacheString(
    newlyCalculatedSpots: SurfSpot[],
    previouslyCachedSpots: SurfSpot[]
  ) {
    let spotsFromCacheThatAreUnchanged = previouslyCachedSpots.filter(
      (value) =>
        !newlyCalculatedSpots.map((s) => s.placeId).includes(value.placeId)
    );
    let allSpotsToCache =
      spotsFromCacheThatAreUnchanged.concat(newlyCalculatedSpots);
    return JSON.stringify(allSpotsToCache);
  }

  private _calculateDistanceFromOriginToSpots(
    origin: GoogleCoordinates,
    surfSpots: SurfSpot[]
  ): Observable<SurfSpot[]> {
    const chunkSize = 25;
    ...
    for (let i = 0; i < surfSpots.length; i += chunkSize) {
      ...
      chunkedServiceObservables.push(
        from(this._calculateDistanceFromOriginToSpotsChunk(origin, chunk))
      );
    }
    return concat(...chunkedServiceObservables);
  }

  private async _calculateDistanceFromOriginToSpotsChunk(
    origin: GoogleCoordinates,
    surfSpots: SurfSpot[]
  ) {
    let request = DistanceMatrixService._buildRequestForSurfSpots(
      origin,
      surfSpots
    );
  ...
  private _enrichSurfSpotsWithDistanceFromResponse(
    surfSpots: SurfSpot[],
    distanceResponse: google.maps.DistanceMatrixResponse
  ) {
    const distances = distanceResponse.rows[0].elements.map(
      (element) => element?.distance ?? undefined
    );
    return surfSpots.map((surfspot, id) => ({
      ...surfspot,
      distanceToCurrentLocation: distances[id],
    }));
  }

  private static _buildRequestForSurfSpots(
    origin: GoogleCoordinates,
    surfSpots: SurfSpot[]
